    ...
    def __init__(self, configs, conv_kernel=[12, 16]):
        ...
        for ii in conv_kernel:
            if ii % 2 == 0:  # the kernel of decomposition operation must be odd
                ...
                isometric_kernel.append((configs.seq_len + ii) // ii)
            else:
                decomp_kernel.append(ii)
                isometric_kernel.append((configs.seq_len + ii - 1) // ii)

        self.seq_len = configs.seq_len
        self.num_class = configs.num_class
        self.d_model = configs.d_model

        # 这句如果有GPU默认使用GPU0，device = torch.device("cuda:0")# 显式指定第一个GPU
        ...
        self.conv_trans = SeasonalPrediction(embedding_size=configs.d_model, n_heads=configs.n_heads,
                                             ...
                                             isometric_kernel=isometric_kernel, device=self.device)
