...
class PortfolioStrategy:
    def __init__(self):
        ...
        # 初始化数据:读取原始数据，并分类，做成序列数据(numpy)
        self.featureList = self.technicalFactor #训练的特征
        self.nSeqLen = 60
        self.train_x, self.train_y,self.test_x,self.test_y,self.featureNum = self.initData(nSeqLen=self.nSeqLen, featureList=self.featureList)

    def initData(self,nSeqLen,featureList):
        # 生成训练数据(训练集，测试集,特征数)
        ...
        return train_x, train_y,test_x,test_y,featureNum
    ...
    def train_model(self,model,epochs,dl_train,dl_valid,trial):
        """
        训练函数
        :param model: 模型
        :param epochs: 训练轮数
    ...
    def objective(self,trial):
        ...
        train_x, train_y,test_x,test_y,featureNum = self.train_x, self.train_y,self.test_x,self.test_y,self.featureNum
        ...
        #drop_last = True,如果数据集大小不能被batch_size整除，则丢弃最后一个batch
        #train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,drop_last=True,sampler=sampler)
        train_loader = DataLoader(train_dataset, batch_size=batch_size,drop_last=True,num_workers=4,sampler=sampler)# sampler时不能用shuffle
        ...
        # TimesNet模型调用
        #configs = TimesNet_configs(seq_len=nSeqLen,enc_in=featureNum)
        configs = TimesNet_configs2(seq_len=nSeqLen,enc_in=featureNum,d_model=params['d_model'],d_ff=params['d_ff'],num_kernels=params['num_kernels'],
                                    top_k=params['top_k'],e_layers=params['e_layers'],freq=params['freq'],dropout=params['dropout'],num_class=2)
        model = CTimesNet(configs)
        ...
        #weight_decay 正则化参数
        #model.optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-5)
        model.optimizer = torch.optim.AdamW(model.parameters(), lr=params["learning_rate"], weight_decay=params["weight_decay"])
    ...
    def optimize_hyperparameters(self):
        ...
        try:
            ...
            print("Install plotly and matplotlib to visualize results")


if __name__ == '__main__':
    ps = PortfolioStrategy()
    #ps.myMain()
    ps.optimize_hyperparameters()#优化参数