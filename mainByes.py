...
class PortfolioStrategy:
    ...
    def modelEvaluation2(self,ytest,ypred):
        ...
        c1precision = report['1']['precision']
        c1recall = report['1']['recall']
        c1F1score = report['1']['f1-score']
        #测试集准确率,精确率，召回率，F1，分类报告：c1precision,c1recall,c1F1score
        return [testAcc,fprecision,frecall,F1score,c1precision,c1recall,c1F1score]
    ...
    def myMain(self):
        ...
        model = BiGRU_Model(input_dim=featureNum,hidden_dim=64,num_layers=2,output_dim=2).to(self.device)
    ...
    def myMain_Byes(self,lr,batchIndex,hiddenDimIndex,num_layers_index,weight_decay):
        """
            "lr": (0.001, 0.01), #学习率
            'batchIndex':(0,2),  #batch_size索引
            "hiddenDimIndex": (0, 2),#隐藏层维度
            "num_layers_index": (1, 5), #隐藏层数量
            "epochs": (100, 500), #训练轮数
        :return:
        """

        # 合并技术指标因子和Alpha191因子，加上基础行情数据
        featureList = ['HT_TRENDMODE', 'DEMA60', 'OBV', 'WILLR', 'TRANGE', 'AROONUP', 'AROONOSC', 'fastdRSI', 'fastkRSI',
                       'AROONDOWN', 'ADX5', 'MFI', 'HT_TRENDLINE', 'BOP', 'HT_SINE', 'FASTK', 'DX', 'HT_DCPERIOD', 'SLOWD',
                       'iVolume', 'SLOWK']
        ...
        batch_size = batch_list[round(batchIndex)]
        #drop_last = True,如果数据集大小不能被batch_size整除，则丢弃最后一个batch
        #train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,drop_last=True,sampler=sampler)
        train_loader = DataLoader(train_dataset, batch_size=batch_size,drop_last=True,sampler=sampler)# sampler时不能用shuffle
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, drop_last=True)
        ...
        hidden_dim = hidden_dim_list[round(hiddenDimIndex)]#隐藏层数量

        num_layers = round(num_layers_index)#层数
        #model = GRU_Model(input_dim=featureNum,hidden_dim=64,num_layers=2,output_dim=2).to(self.device)
        model = BiGRU_Model(input_dim=featureNum,hidden_dim=hidden_dim,num_layers=num_layers,output_dim=2).to(self.device)
        ...
        ypred,ytest = self.PredictionEvaluation(model,test_loader)
        #self.modelEvaluation("测试集上评估模型",ytest,ypred)
        testAcc,testPrecision,testRecall,testF1 ,c1precision,c1recall,c1f1 = self.modelEvaluation2(ytest,ypred)

        return c1f1


    def ByesOptimize(self):
        """
            贝叶斯超参数优化
        :return:
        """
        params = {
            "lr": (0.001, 0.01), #学习率
            'batchIndex':(0,3),  #batch_size索引
            "hiddenDimIndex": (0, 3),#隐藏层维度
            "num_layers_index": (1, 5), #隐藏层数量
            "weight_decay": (1e-4, 1e-2), #权重衰减
        }
        # 贝叶斯优化
        optimizer = BayesianOptimization(f=self.myMain_Byes, pbounds=params,verbose=2, random_state=1)
        try:
