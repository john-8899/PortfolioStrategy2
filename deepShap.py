...
class PortfolioStrategy:
    def __init__(self,isPrintTrainlog=True):
        self.isPrintTrainlog = isPrintTrainlog
        # 技术指标因子
    ...
    def train_model(self,model,epochs,dl_train,dl_valid,isEarlyStopping=True):
        ...
        for epoch in range(1,epochs+1):
            ...
            # 早停机制
            early_stop(loss_sum/step,model)
    ...
    def modelEvaluation2(self,ytest,ypred):
        """
        评估分类模型的性能指标
        ...
        F1score = round(f1_score(ytest,ypred,labels=None,pos_label=1,average='macro',sample_weight=None),3) #F1
        # 生成包含详细分类指标的字典报告
        report = classification_report(ytest,ypred,zero_division=1,output_dict=True)
        c1f1 = report['1']['f1-score']
        c1prec = report['1']['precision']
        c1recall = report['1']['recall']
        # 返回所有评估指标:测试集准确率,F1,c1prec,c1recall,c1f1
        return [testAcc,F1score,c1prec,c1recall,c1f1]
    ...
    def shap_analysis(self,model,featureList,train_x,test_x,clase_id):
        """
        使用SHAP (SHapley Additive exPlanations) 方法分析模型特征重要性

        参数:
            model: 待分析的深度学习模型
            featureList: 特征名称列表，与输入数据的特征维度对应
            train_x: 训练数据，用于初始化SHAP解释器
            test_x: 测试数据，用于计算SHAP值
            clase_id: 目标类别ID，指定要分析的具体类别

        返回:
            dict_shap: 字典形式返回各特征对指定类别预测的贡献度，
                      键为特征名，值为对应的SHAP绝对值均值
        """
        # 将输入数据转换为PyTorch张量
        devie = next(model.parameters()).device
        train_x_tensor = torch.tensor(train_x, dtype=torch.float32).to(devie)
        test_x_tensor = torch.tensor(test_x, dtype=torch.float32).to(devie)

        # 初始化DeepExplainer并计算SHAP值
        # shap_values形状: (batch, time, features, classes)
        explainer = shap.DeepExplainer(model, train_x_tensor[:1000])#
        # 计算shap值:  shap_values.shape)# (1,60,92,4) batch × time × features × classes。
        shap_values = explainer.shap_values(test_x_tensor[:100],check_additivity=False)

        # 计算批次维度上的绝对SHAP值均值
        # 输出形状: (time, features, classes)
        shap_values = np.mean(np.abs(shap_values),axis=0)#(batch × time × features × classes) ---> time × features × classes ,batch行求均值

        # 聚合时间维度并提取指定类别的特征贡献
        # 输出形状: (features,)
        shap_last_values = np.sum(shap_values[:,:,clase_id],axis=0)# time × features × classes ---> features × 1 #取某一类的特征

        # 构建特征名称到SHAP值的映射字典
        dict_shap = {feature: float(shap_last_values[i]) for i,feature in enumerate(featureList)}

        return dict_shap


    def myMain_train_shap(self,featureList):
        # 生成训练数据(训练集，测试集,特征数)
        nSeqLen = 60
        ...
        # 创建模型
        model = GRU_Model(input_dim=featureNum,hidden_dim=64,num_layers=2,output_dim=2).to(self.device)
        ...
        # 计算SHAP值,提取1类的特征贡献
        dict_shap_values = self.shap_analysis(model,featureList,train_x,test_x,1)

        return dict_shap_values

    def myMain_train(self,featureList):
        # 生成训练数据(训练集，测试集,特征数)
        nSeqLen = 60
        train_x, train_y,test_x,test_y,featureNum = self.makeTrainData(nLen=nSeqLen,featureList=featureList)
        #print("特征数：%d,训练集个数:%d,测试集个数：%d"%(featureNum,len(train_x),len(test_x)))
        ...
        ypred,ytest = self.PredictionEvaluation(model,test_loader)
        #
        acccuracy,f1,c1precision,c1recall,c1f1 =self.modelEvaluation2(ytest,ypred)

        return acccuracy,f1,c1precision,c1recall,c1f1
