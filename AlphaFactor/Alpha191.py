...
def alpha191_27(close_df, n=12):
    ...
    # 替换零为一个小常数（如1e-8）
    close_shift_3 = close_df.shift(3).replace(0, 1e-8)
    close_shift_6 = close_df.shift(6).replace(0, 1e-8)

    # 重新计算
    expr = ((close_df - close_shift_3) / close_shift_3 * 100) + \
           ((close_df - close_shift_6) / close_shift_6 * 100)

    result = ta.WMA(expr, n)
    return result
...
def alpha191_28(close_df, high_df, low_df, n=9):
    ...
    :return:
    """
    return 3 * sma((close_df - ta.MIN(low_df, n)) / (ta.MAX(high_df, n) - ta.MIN(low_df, n)+1e-8) * 100, 3,
                   1) - 2 * sma(
        sma((close_df - ta.MIN(low_df, n)) / (ta.MAX(high_df, n) - ta.MIN(low_df, n)+1e-8) * 100, 3, 1), 3, 1)
...
def alpha191_30(close_df, mkt_df, smb_df, hml_df, n=60, m=20):
    ...
    :param close_df:
    :param mkt_df:MKT 因子：MKT 因子定义为 RM - RF，其中 RM 为全市场股票市值加权收益率，RF 是无风险利率。可以选取中证全指等指数来代表全市场，计算其成分股的市值加权收益率作为 RM，无风险利率 RF 可以用 10 年期国债到期收益率等近似代替，然后计算 MKT = RM - RF。
    :param smb_df:SMB 因子：每年五月末，将股票按市值等分为两组 Big（B）、Small（S）。计算这两组股票组合的市值加权收益率，SMB 等于小市值组（S）的平均收益率减去大市值组（B）的平均收益率，即 SMB = 1/2（S 组收益率）- 1/2（B 组收益率）。
    :param hml_df:HML 因子：每年五月末，将账面市值比按 30%、40%、30% 分为三组 Low（L）、Middle（M）、High（H）。计算 HML 因子，即 HML =（SH + BH）/2 -（SL + BL）/2，其中 SH、BH 分别是小市值高账面市值比组合和大市值高账面市值比组合的收益率，SL、BL 分别是小市值低账面市值比组合和大市值低账面市值比组合的收益率。
    :param n: default 60
    :param m: default 20
    :return:
    """
...
def alpha191_47(close_df, high_df, low_df, n=6, m=9):
    ...
    #修改后的公式
    # 计算最大值和最小值
    max_high = ta.MAX(high_df, n)
    min_low = ta.MIN(low_df, n)
    ...
    # 主体计算
    value = (max_high - close_df) / denominator * 100

    # 加权平均
    result = sma(value, m, 1)
...
def alpha191_55(close_df, open_df, high_df, low_df, n=20):
    ...
    denom = np.maximum(denom, 1e-8) #  防止分母为零
    # 计算分子
...
def alpha191_57(close_df, high_df, low_df, n=9):
    ...
    :return:
    """
    return sma((close_df - ta.MIN(low_df, n)) / (ta.MAX(high_df, n) - ta.MIN(low_df, n)+1e-8) * 100, 3, 1)
...
def alpha191_72(close_df, high_df, low_df, n=15):
    ...
    :return:
    """
    return sma((ta.MAX(high_df, 6) - close_df) / (ta.MAX(high_df, 6) - ta.MIN(low_df, 6)+1e-8) * 100, n, 1)
...
def alpha191_75(close_df, open_df, benchmark_close_df, benchmark_open_df, n=50):
    ...
    :param close_df:
    :param open_df:
    :param benchmark_close_df: 标准指数收盘价
    :param benchmark_open_df: 标准指数开盘价
    :param n: default 50
    :return:
    """
...
def alpha191_76(close_df, volume_df, n1=20, n2=20):
    ...
    :return:
    """
    # return (ta.STDDEV(np.abs((close_df / delay(close_df, 1) - 1)) / zero_to_one(volume_df/100), n1) /
    #         ta.MA(np.abs((close_df / delay(close_df, 1) - 1)) / zero_to_one(volume_df/100), n2))
    # return (ta.STDDEV(np.abs((close_df / delay(close_df, 1) - 1)) , n1) /
    #         ta.MA(np.abs((close_df / delay(close_df, 1) - 1+1e-8)) , n2))

    delay_close = close_df.shift(1)
    returns = (close_df / delay_close - 1).fillna(0)

    # 添加微小量防止除零
    returns_abs = np.abs(returns) + 1e-8

    std_dev = returns_abs.rolling(n1).std()
    mean_val = returns_abs.rolling(n2).mean()

    # 避免分母为零
    result = std_dev / mean_val.replace(0, 1e-8)
...
def alpha191_82(close_df, high_df, low_df, n1=6, n2=20):
    ...
    :return:
    """
    return sma((ta.MAX(high_df, n1) - close_df) / (ta.MAX(high_df, n1) - ta.MIN(low_df, n1)+1e-8) * 100, n2, 1)
...
def alpha191_96(close_df, high_df, low_df, n1=9, n2=3):
    ...
    :return:
    """
    return sma(sma((close_df - ta.MIN(low_df, n1)) / (ta.MAX(high_df, n1) - ta.MIN(low_df, n1)+1e-8) * 100, n2),
               n2)
...
def alpha191_117(close_df, high_df, low_df, volume_df, ret_df=None, n=32, m=16):
    """
...
def alpha191_119(open_df, vwap_df, volume_df, n=5, m=26):
    ...
    """
    # 修改后：提取特定列或降维
    corr_result = ta.CORREL(vwap_df.squeeze(), ta.SUM(ta.MA(volume_df.squeeze(), n), m), 5).squeeze()

    # 修改 alpha191_119 函数中的计算逻辑，确保降维操作
    corr_value = ta.CORREL(rank(open_df.squeeze()), rank(ta.MA(volume_df.squeeze(), 15)), 21).squeeze()
    min_matrix = np.minimum(corr_value, 9)  # 确保输入为一维
    tsrank_result = tsrank(min_matrix, 7)

    return (rank(decaylinear(corr_result, 7).squeeze()) - rank(decaylinear(tsrank_result, 8).squeeze()))
...
def alpha191_122(close_df, n=13):
    ...
    sma1 = sma(log_close, n, 2)
    sma2 = sma(sma1, n, 2)
    sma3 = sma(sma2, n, 2)

    # 计算延迟值
    delayed_sma3 = delay(sma3, 1)

    # 添加微小量避免除零错误
    denominator = delayed_sma3 + 1e-8

    # 计算最终结果
    result = (sma3 - delayed_sma3) / denominator
...
def alpha191_127(close_df, n=12):
    ...
    # 计算 CLOSE 的 n 日最大值
    max_close = close_df.rolling(window=n).max()

    # 防止分母为零，加入一个微小量
    max_close = np.where(max_close == 0, 1e-8, max_close)

    # 计算公式的核心部分
    ratio = 100 * (close_df - max_close) / max_close

    # 对平方后的结果取均值，再开方
    result = np.sqrt((ratio ** 2).rolling(window=n).mean())
...
def alpha191_135(close_df, n=20):
    ...
    :return:
    """
    close_diff = delay(close_df, n)
    close_diff = np.where(close_diff == 0, 1e-8, close_diff)
    return sma(delay(close_df / close_diff, 1), n, 1)
...
def alpha191_143(close_df):
    ...
    #
    # return close_df1
    adj_values = np.zeros_like(close_df)  # 初始化调整值数组
    adj_values[0] = 1.0  # 初始化为1，避免零乘问题
    #  防止除零,
    close_df = np.where(close_df==0, 1e-8, close_df)
    for i in range(1, len(close_df)):
        if close_df[i] > close_df[i-1]:  # 价格上涨
            ret = (close_df[i] - close_df[i-1]) / close_df[i-1]  # 计算收益率
            adj_values[i] = adj_values[i-1] * (1 + ret)  # 累积乘以前一日值
        else:
            adj_values[i] = adj_values[i-1]  # 维持前一日值
...
def alpha191_144(close_df, amount_df, n=20):
    ...
    :return:
    """
    # close_shifted = delay(close_df, 1).fillna(1e-8)  # DELAY(CLOSE, 1)
    # close_condition = close_df < close_shifted  # CLOSE < DELAY(CLOSE, 1)
    #
    # # 防止分母为零
    # amount_df = np.where(amount_df == 0, 1e-8, amount_df)
    #
    ...
    # # 处理 count_result 为零的情况
    # count_result[count_result == 0] = 1e-8
    #
    # return sumif_result / count_result
    # 计算延迟值
    close_shifted = delay(close_df, 1)  # DELAY(CLOSE, 1)

    # 条件判断 CLOSE < DELAY(CLOSE,1)
    close_condition = close_df < close_shifted

    # 防止除以零
    close_shifted = np.where(close_shifted == 0, 1e-8, close_shifted)

    # 计算 ABS(CLOSE/DELAY(CLOSE,1)-1)/AMOUNT
    close_ratio = np.abs((close_df / close_shifted) - 1) / amount_df
    ...
    # 处理分母为零的情况
    count_result = np.where(count_result == 0, 1e-8, count_result)

    # 最终结果
    result = sumif_result / count_result
    return result
...
def alpha191_146(close_df, n1=20, n2=61, n3=60, m=2):
    ...
    #出现0的情况，用1e-8代替
    sma_diff_squared = np.where(sma_diff_squared == 0, 1e-8, sma_diff_squared)
...
def alpha191_157(close_df, high_df, volume_df, vwap_df, n=30, m=37):
    ...
    return np.where(corr1_rank < corr2_rank, 1, 0)
...
def alpha191_158(close_df, high_df, low_df, n=15, m=2):
    ...
    return ((high_df - close_sma) - (low_df - close_sma)) / close_df
...
def alpha191_160(close_df, n=20, m=1):
    ...
    # close_shifted = delay(close_std, 1)
    # value = (close_df <= close_shifted).astype(int)
    # value = value * close_std
    # return sma(value, n, m)
    # 1. 计算条件判断
    condition = (close_df <= delay(close_df, 1))

    # 2. 计算条件标准差
    std_values = np.where(condition, ta.STDDEV(close_df, n), 0)

    # 3. 处理可能的nan值
    std_values = np.nan_to_num(std_values)

    # 4. 计算SMA
    return sma(std_values, n, m)
...
def alpha191_166(close_df, n=20):
    ...
    """
    # 计算公式中的各个部分
    close_diff = delay(close_df, 1)
    close_diff = np.where(close_diff == 0, 1e-8, close_diff)
    part1 = close_df / close_diff - 1
    part2 = ta.MA(close_df / close_diff - 1, n)
    part3 = ta.SUM(close_df / close_diff, 20) ** 2
...
def alpha191_169(close_df, n=9, m=12):
    ...
    part2 = sma(part1, m, 1)
    part3 = sma(part1, 26, 1)

    # 计算最终结果
    result = sma(part2 - part3, 10, 1)
...
def alpha191_173(close_df, n=13):
    ...
    :return:
    """
    close_df = close_df.fillna(method='ffill').replace(0, 1e-8)
    # 计算公式中的各个部分
...
def alpha191_186(close_df, high_df, low_df, n=14, m=6):
    ...
    # 出现0时，将SUM(TR,14)设为1e-8
    sum_tr = np.where(sum_tr == 0, 1e-8, sum_tr)
    # 计算sum_ld*100/sum_tr
